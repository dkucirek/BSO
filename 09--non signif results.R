...
# 09_generate_nonsignificant_results.R

# Load libraries
library(tidyverse)

# Source analysis scripts to run them if results are not already in the environment
if (!exists("complication_fisher_results")) {
  source("03- analysis for complications.R")
}
if (!exists("oophorectomy_fisher_results")) {
  source("04- ooph analysis.R")
}
if (!exists("readmission_fisher_results")) {
...
nonsignificant_results <- tidy_results %>%
  filter(p_value > 0.05) %>%
  select(Analysis, p_value_formatted, method)

# --- Save Non-Significant Results ---

write_csv(nonsignificant_results, "nonsignificant_results.csv")

cat("Non-significant results saved to nonsignificant_results.csv\n")
